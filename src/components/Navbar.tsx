      ...
      <div className="container mx-auto px-4 h-14 sm:h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          {user ? (
            ...
            <Link to="/login" className="px-4 py-2 rounded-lg bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 transition-all hover:shadow-[0_0_20px_rgba(255,77,0,0.3)]">
              Entrar
            </Link>
          )}
        </div>
          ...
          <div className="pt-2 border-t border-border mt-2">
            {user ? (
              <>
                ...
                <Link to="/dashboard" className="block py-3 px-3 text-muted-foreground hover:text-foreground hover:bg-secondary/50 rounded-lg" onClick={() => setMobileOpen(false)}>Meu Painel</Link>
                <Link to="/dashboard/configuracoes" className="block py-3 px-3 text-muted-foreground hover:text-foreground hover:bg-secondary/50 rounded-lg" onClick={() => setMobileOpen(false)}>Configurações</Link>
                <button onClick={() => { handleSignOut(); setMobileOpen(false); }} className="block w-full text-left py-3 px-3 text-destructive hover:bg-destructive/10 rounded-lg">Sair</button>
              </>
            ) : (
              <Link to="/login" className="block py-3 px-4 rounded-xl bg-primary text-primary-foreground font-semibold text-center" onClick={() => setMobileOpen(false)}>Entrar</Link>
            )}
          </div>
        </div>
      )}

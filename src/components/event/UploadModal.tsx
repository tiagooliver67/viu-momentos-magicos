...
export default function UploadModal({ open, onClose, onUpload, isUploading, type }: Props) {
  ...
  const accept = type === "photos" ? "image/jpeg,image/png,image/webp" : "video/mp4,video/mov,video/avi";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="glass-card p-6 max-w-lg w-full" onClick={e => e.stopPropagation()}>
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">{type === "photos" ? "JPG, PNG, WEBP • Máximo 30MB por arquivo" : "MP4, MOV, AVI • Máximo 100MB por arquivo"}</p>

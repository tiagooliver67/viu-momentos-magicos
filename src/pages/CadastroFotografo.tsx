...
export default function CadastroFotografo() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.fullName || !form.email || !form.password || !form.cpfCnpj || !form.state || !form.experienceLevel) {
        ...
        <div className="w-full max-w-lg">
          <div className="bg-card border border-border rounded-2xl p-8">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl font-bold text-foreground">Cadastro de Fotógrafo</h1>
                <p className="text-muted-foreground text-sm">Crie sua conta profissional</p>
              </div>
            </div>
            <p className="text-muted-foreground text-sm mb-6">
              Já tem conta?{" "}
              <Link to="/login/fotografo" className="text-primary font-semibold hover:underline">Fazer login</Link>
            </p>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <Input placeholder="Rede social (opcional)" value={form.socialMedia} onChange={e => update("socialMedia", e.target.value)} className="h-12" />
              ...
              <Button type="submit" className="w-full h-12 text-base font-bold" disabled={loading || !agreed}>
                {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Criar conta de Fotógrafo"}
              </Button>
            </form>
          </div>
        </div>

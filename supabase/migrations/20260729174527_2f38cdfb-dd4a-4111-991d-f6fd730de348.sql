REVOKE EXECUTE ON FUNCTION public.is_proposal_party(uuid) FROM anon;
...
BEGIN
  IF auth.uid() IS NOT NULL AND auth.uid() <> _user_id AND NOT public.is_super_admin() THEN
...
REVOKE EXECUTE ON FUNCTION public.recalc_photographer_level(uuid) FROM PUBLIC, anon;

...
import {
  ...
  Clock, FileText, Receipt, Calendar, Crown, Filter, Download
} from "lucide-react";
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-6 lg:p-8 max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <button className="px-5 py-2.5 rounded-xl border border-border text-sm font-medium text-muted-foreground hover:bg-secondary transition-colors">
              Dúvidas em relação ao financeiro? <span className="font-bold text-foreground">Clique aqui</span>
            </button>
          </div>

          {/* Two cards: Minhas Vendas + Meu Extrato */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            {/* Minhas vendas */}
            <div className="glass-card p-6">
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-border">
                <h2 className="text-lg font-bold text-foreground">Minhas vendas</h2>
                <button onClick={() => setShowSales(!showSales)} className="text-muted-foreground hover:text-foreground transition-colors">
            ...
            <div className="glass-card p-6">
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-border">
                <h2 className="text-lg font-bold text-foreground">Meu Extrato</h2>
                <button onClick={() => setShowExtract(!showExtract)} className="text-muted-foreground hover:text-foreground transition-colors">
              ...
              {showExtract ? (
                <div className="space-y-3">
                  ...
                  <button className="mt-4 w-full flex items-center justify-between bg-primary text-primary-foreground rounded-xl px-5 py-3 font-bold text-sm hover:bg-primary/90 transition-all hover:shadow-[0_0_20px_rgba(255,77,0,0.3)]">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 mb-10">
            {quickActions.map((action) => (
              <button
                key={action.label}
                className="glass-card flex flex-col items-center gap-2.5 p-4 hover:scale-105 hover:shadow-lg transition-all group"
              >
                <div className="w-12 h-12 rounded-full border-2 border-primary/20 flex items-center justify-center group-hover:border-primary/50 transition-colors">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8">
            ...
            <button className="px-8 py-2.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm hover:bg-primary/90 transition-all mt-4 sm:mt-6">
              ...
              <table className="w-full text-sm">
                ...
                <tfoot>
                  <tr className="border-t-2 border-border">
                    <td colSpan={4} className="p-4 text-right font-bold text-muted-foreground uppercase text-sm">
                      Total
                    </td>
